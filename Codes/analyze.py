...
class PCIlz:
    ...
    def plot_source_activations(self, timepoints: List[float], 
        ...
        for actual_time in actual_times:
            ...
            if save_path:
                brain.save_image(os.path.join(os.path.dirname(__file__), '..', 'Results', f"{save_path}_time_{int(actual_time*1000)}ms.png"))
    ...
    def plot_tms_eeg_analysis(self, SS: np.ndarray, pci: float, 
        ...
        if save_path:
            plt.savefig(os.path.join(os.path.dirname(__file__), '..', 'Results', save_path), dpi=300, bbox_inches='tight')
